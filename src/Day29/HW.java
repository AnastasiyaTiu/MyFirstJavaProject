...
public class HW {
    public static void main(String[] args) {
        ...
        //TASK 1. Print all  - hint ( substring, indexOf)

        System.out.println(Arrays.toString(employees));
        System.out.print("Job titles: ");
        for (String eachEmployee: employees) {
            int index = eachEmployee.indexOf("[");
            int lIndex = eachEmployee.lastIndexOf("]");
            String job = eachEmployee.substring(index + 1, lIndex);
            System.out.print(job + ", ");
        }
//TASK 2: Print all
       /*for example :
        Philipa-Salthouse Electrical Engineer
        Ulrika-Golding Nuclear Power Engineer */
        for (String eachEmployee: employees) {
            int index = eachEmployee.indexOf("[");
            int lIndex = eachEmployee.lastIndexOf("]");
            String job = eachEmployee.substring(index + 1, lIndex);
            System.out.println("\n FirstName-lastName jobTitle: ");
            String firstName = eachEmployee.substring(0, eachEmployee.indexOf(" "));
        String lastName = eachEmployee.substring(eachEmployee.indexOf(" ")+1, index-1);
            System.out.println(firstName + "-" +lastName+" "+job);
        }
//TASK 3. Look for those 2 people in the array and assert below data
        ...
        if true{
            ...
            Print "FAIL: Verification error:  Tracy Bartle's job is NOT Geological Engineer
                    */
        for (String twoEmployees: employees) {
            if (twoEmployees.toLowerCase().contains("Tracy Bartle") && twoEmployees.toLowerCase().contains("Geological Engineer")){
            ...
            if (twoEmployees.toLowerCase().contains("Skipper Abrahamsson") && twoEmployees.toLowerCase().contains("Financial Advisor")){
